echo "Installing required testing tools (socket.io-client, jsonwebtoken)..."
npm install socket.io-client jsonwebtoken dotenv --no-save > /dev/null 2>&1

echo "Creating verify_production.mjs..."
cat << 'INNER_EOF' > verify_production.mjs
...
async function runHealthCheck() {
  ...
  try {
    ...
    if (testUsers.length < 2) {
      console.log("❌ Need at least 2 active users to simulate chat/notifications.");
      process.exit(1);
    }
    ...
    // Save original role and temporarily elevate to Premium to bypass Free user limits
    ...
    // Backend uniquely expects EXACT { id, email, role, type: 'access' } payload
    ...
    receiverSocket.on("message:receive", (msg) => {
      ...
      console.log("✅ Notification Triggers: Validated");
      console.log("==========================================\n");
    ...
    setTimeout(async () => {
      await prisma.user.update({ where: { id: sender.id }, data: { role: originalRole } });
      console.error("\n❌ TIMEOUT: Did not receive expected real-time events. (User might be blocked or missing).");
      process.exit(1);
    }, 10000);

  } catch (err) {
    if (originalRole) {
       await prisma.user.update({ where: { id: 1 }, data: { role: originalRole } });
    }
...
node verify_production.mjs

echo "Cleaning up..."
rm verify_production.mjs
