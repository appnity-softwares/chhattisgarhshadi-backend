...
export const sendMessage = async (
  ...
  try {
    ...
    // Calculate Limit Responses using IST standard
    const istOffset = 5.5 * 60 * 60 * 1000;
    const dateKey = new Date(Date.now() + istOffset).toISOString().split('T')[0];
    ...
    return { 
      ...
      isDuplicate: false,
      limitsInfo: {
        canSendMessage: (limit === -1 || isMatched) ? true : messagesUsed < limit,
        remainingMessages,
